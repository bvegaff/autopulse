pro fit_transit_check_transit, $
                 ...
                 plot_transit_detrends=plot_transit_detrends  ;must have transit_times.txt be for the depth/dur you want to investigate (i.e. run single_depth_dur before this)
...
;Added to check out the polyfits
fit_window=dblarr(nt)
...
;        print,iseg,itime
;;=============================================================================
;;6.2.1.1a If itime is the beginning of a transit, plot each of our polynomial fits in the window
;;=============================================================================
        if keyword_set(plot_transit_detrends) then begin
            indx_transit = where(begin_transit_cadences eq itime) ;indx_transit ne -1 where we start a transit
        endif
;;=============================================================================
...
;;=============================================================================
;;6.2.1.2.2.1a Plot if we're at a beginning of a transit
                if keyword_set(plot_transit_detrends) then begin
                    if indx_transit ne -1 then begin
                        ploterror,time[indx],flux,ssap[indx],tit='Depth='+string(depth[1]*1d6,format='(f8.1)')+ $
                        'ppm; Duration='+string(tdur[idur]*24d0,format='(f4.1)')+ $
                        'hr; fits at t_transit= '+string(time[itime],format='(f6.1)'),$
                        color=colors[0],/YNOZERO,errcolor=colors[0];,yrange=[0.98,1.02]
